...
组件用轻量的 per-column masonry 排列卡片，不是普通按行 grid，也不是 CSS `column-count` 伪瀑布。

- 列数随视口变化：移动端 1 列，`md`（≥768px）2 列，`lg`（≥1024px）3 列。
- 每列内部是纵向卡片流，列与列之间等宽；列间距与卡片纵向间距都是 24px。
- 下一张卡片会放进当前估算最短的列。高度估算只按封面比例、标题字数、摘要行数、标签数量推算，用来挑列，不会设置卡片真实高度。

SSR / 首屏安全：

- `columnCount` 默认 `1`，SSR 与首屏保持单列纵向流，不在 setup 顶层读取 `window`。
- 列数与容器宽度只在 `onMounted` 里通过 `matchMedia` 与 `ResizeObserver` 读取，卸载时清理监听。
- 视口宽度变化（横竖屏、窗口缩放）会自动重算列数与最短列分布。

视觉与交互保持不变：卡片 hover 聚光、RouterLink 跳转、tag 点击（`@click.stop` 阻止冒泡到卡片）、游戏卡片元信息布局都沿用现有规则。
